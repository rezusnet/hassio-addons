...
- **Host:** `2eafa696_valkey` (or your add-on slug)
- **Port:** `6379` (or whatever you configured)
- **Password:** (if set in Configuration)

Example connection string: `valkey://:<password>@2eafa696_valkey:6379/0`
...
1. Install and start the Valkey add-on (no password needed for internal communication).
2. In the Mastodon add-on configuration, set `redis_host` to the Valkey add-on slug (e.g., `2eafa696_valkey`).
