...
export LOCAL_DOMAIN="$(bashio::config 'LOCAL_DOMAIN')"
...
if bashio::config.has_value 'SMTP_SERVER'; then
    ...
    export SMTP_LOGIN="$(bashio::config 'SMTP_LOGIN')"
    export SMTP_PASSWORD="$(bashio::config 'SMTP_PASSWORD')"
    export SMTP_FROM_ADDRESS="$(bashio::config 'SMTP_FROM_ADDRESS')"
fi
...
generate_vapid_keys() {
    ...
    if bashio::config.has_value 'VAPID_PRIVATE_KEY' && bashio::config.has_value 'VAPID_PUBLIC_KEY'; then
        ...
        export VAPID_PRIVATE_KEY="$(cat "${vapid_private_file}")"
        export VAPID_PUBLIC_KEY="$(cat "${vapid_public_file}")"
        ...
        if command -v ruby > /dev/null 2>&1; then
            ...
            if [ -n "${vapid_output}" ]; then
                export VAPID_PRIVATE_KEY="$(echo "${vapid_output}" | head -1)"
                export VAPID_PUBLIC_KEY="$(echo "${vapid_output}" | tail -1)"
            fi
        fi
        if [ -z "${VAPID_PRIVATE_KEY:-}" ] || [ -z "${VAPID_PUBLIC_KEY:-}" ]; then
            export VAPID_PRIVATE_KEY="$(openssl rand -base64 32 | tr -d '\n')"
            export VAPID_PUBLIC_KEY="$(openssl rand -base64 32 | tr -d '\n')"
